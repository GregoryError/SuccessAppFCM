...
void MyClient::slotReadyRead()
{
    ...
    if(m_ptxtInfo.length() > 10 && m_ptxtInfo.mid(0, 11) == "getAllData!")
    {
        ...
        qDebug() << "launch_ind: " << dataSet.value("value").toInt();
...
{
    this->sendMsgs("Заявка передана через мобильное приложение: " + issue_msg + " Жду Вашей помощи!");
    issue_msg.clear();
}

bool MyClient::is_Show_Val_Quest()
{
    return dataSet.value("is_Show_Val_Wnd").toBool() && dataSet.value("value").toInt() % 10 == 0;
}
